...
function kiss_wse_initialize_exporter(): void {
    ...
    if ( ! class_exists( 'KISS_WSE_Rule_Parser' ) ) {
        class KISS_WSE_Rule_Parser {
            ...
            public function format_rule_as_html(array $rule): string {
                // ... (This formatting logic is unchanged)
                $html = '';
                $condition_strings = [];
    ...
    if ( ! class_exists( 'KISS_WSE_Exporter' ) ) {
        final class KISS_WSE_Exporter {
            // ... (The entire KISS_WSE_Exporter class is unchanged from v1.5.1)
            private $page_slug = 'kiss-wse-export';
            public function __construct() { add_action( 'admin_menu', [ $this, 'register_menu' ] ); add_action( 'admin_post_' . $this->page_slug, [ $this, 'handle_export' ] ); add_filter( 'plugin_action_links_' . plugin_basename( KISS_WSE_PLUGIN_FILE ), [ $this, 'add_action_links' ] ); }
            public function add_action_links( array $links ): array { $settings_link = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'tools.php?page=' . $this->page_slug ) ), __( 'Export & Scan Settings', 'kiss-woo-shipping-exporter' ) ); array_unshift( $links, $settings_link ); return $links; }
            public function register_menu(): void { add_management_page( 'KISS Woo Shipping Exporter', 'KISS Shipping Exporter', 'manage_woocommerce', $this->page_slug, [ $this, 'render_page' ] ); }
            public function render_page(): void { ?> <div class="wrap"> <h1><?php esc_html_e( 'KISS Woo Shipping Settings Exporter & Scanner', 'kiss-woo-shipping-exporter' ); ?></h1> <hr/> <div id="custom-rules-scanner"> <h2><?php esc_html_e( 'Custom Rules Scanner', 'kiss-woo-shipping-exporter' ); ?></h2> <p><?php esc_html_e( 'The scanner analyzes your active theme files for code-based shipping restrictions and attempts to translate them into a human-readable format.', 'kiss-woo-shipping-exporter' ); ?></p> <?php $this->render_custom_rules_tables(); ?> </div> <hr/> <div id="ui-settings-exporter"> <h2><?php esc_html_e( 'UI-Based Settings Export', 'kiss-woo-shipping-exporter' ); ?></h2> </div> </div> <?php }
            private function render_custom_rules_tables(): void { $files_to_scan = [ 'Theme Functions' => get_stylesheet_directory() . '/functions.php', 'Shipping Restrictions File' => get_stylesheet_directory() . '/inc/shipping-restrictions.php', ]; $rule_parser = new KISS_WSE_Rule_Parser(); foreach ($files_to_scan as $label => $file_path) { echo '<h3>' . esc_html( $label ) . '</h3>'; echo '<code>' . esc_html( str_replace(ABSPATH, '/', $file_path) ) . '</code>'; if ( ! file_exists($file_path) ) { echo '<p><em>' . esc_html__('File not found.', 'kiss-woo-shipping-exporter') . '</em></p>'; continue; } $rules = $rule_parser->parse_file($file_path); echo '<table class="wp-list-table widefat striped" style="margin-top:1em;"><thead><tr><th style="width:45%;">Human-Readable Analysis (Best Guess)</th><th>Source Code Snippet</th></tr></thead><tbody>'; if ( is_wp_error($rules) ) { echo '<tr><td colspan="2"><strong>Parser Error:</strong> ' . esc_html( $rules->get_error_message() ) . '</td></tr>'; } elseif ( empty($rules) ) { echo '<tr><td colspan="2">' . esc_html__('No shipping restriction rules were automatically detected in this file.', 'kiss-woo-shipping-exporter') . '</td></tr>'; } else { foreach ( $rules as $rule ) { echo '<tr>'; echo '<td>' . wp_kses_post( $rule_parser->format_rule_as_html($rule) ) . '</td>'; echo '<td><pre style="white-space: pre-wrap; font-size: 12px; background:#f9f9f9; padding:10px; border:1px solid #ddd;"><code>' . esc_html( $rule['raw_code'] ) . '</code></pre></td>'; echo '</tr>'; } } echo '</tbody></table>'; echo '<details style="margin-top:1em; padding-left: 10px;"><summary style="cursor:pointer; color:#2271b1;">Toggle Debugging Information</summary><div style="padding:10px; background-color:#fafafa; border:1px solid #ddd; margin-top:5px;">'; $debug_info = $rule_parser->get_last_debug_info(); echo '<strong>Parser Status:</strong> ' . esc_html($debug_info['status']) . '<br>'; if (!empty($debug_info['ast_dump'])) { echo '<strong>AST (Abstract Syntax Tree) Dump:</strong><pre style="white-space:pre-wrap; max-height: 400px; overflow-y: scroll; background-color: #fff; padding: 5px;"><code>' . esc_html($debug_info['ast_dump']) . '</code></pre>'; } echo '</div></details>'; } }
            public function handle_export(): void { /* Omitted for brevity */ }
        }
    }
